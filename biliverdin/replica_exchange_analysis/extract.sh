#!/bin/bash
#An old command that might be needed, seems to not normalize the probabilities:
#cphstats -i bla.md1.cpin reordered.cpout.pH_${m} -o pH${m}_calcpka.dat --population pH${m}_populations.dat
#so I will extract the probabilities manually
...
while (( $(bc <<< "$m <= $pH_max") )); do
  ...
  for i in {0..24}; do 
    echo "STATE $i";
    if [ "$i" -eq 0 ]; then
        grep "State:  $i" reordered.cpout.pH_${m} | wc -l; 
        count_dd=$(grep "State:  $i" reordered.cpout.pH_${m} | wc -l) 
        deprot=$((count_dd + deprot))
    elif [ "$i" -ge 1 ] && [ "$i" -le 4 ]; then 
        grep "State:  $i" reordered.cpout.pH_${m} | wc -l; 
        count_left=$(grep "State:  $i" reordered.cpout.pH_${m} | wc -l)
        lefty=$((count_left + lefty))
    elif [ "$i" -ge 5 ] && [ "$i" -le 8 ]; then
        grep "State:  $i" reordered.cpout.pH_${m} | wc -l; 
        count_right=$(grep "State:  $i" reordered.cpout.pH_${m} | wc -l)
        righty=$((count_right + righty))
    else 
        grep "State: $i" reordered.cpout.pH_${m} | wc -l; 
        count_pp=$(grep "State: $i" reordered.cpout.pH_${m} | wc -l)
